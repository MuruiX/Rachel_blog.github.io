...
![1736148950855](image/note/1736148950855.png)
...
1. **顶部柱状图 (Intersection Size)** ：

* 代表不同集合交集中的样本数量（intersection size）。
* **如何查看** ：
  * 每根柱子的高度表示该交集中样本的数量。
  * 柱子越高，表示该交集包含的样本越多。
  * 可以依次观察柱子的分布，找到主要的交集。

1. **底部矩阵 (Intersection Matrix)** ：

* 显示集合的交集关系。
* **如何查看** ：
  *  **黑点** ：表示参与交集的集合。
  *  **连线** ：将黑点连接起来，表示交集包含的多个集合。
  *  **灰色圆点** ：表示未参与该交集的集合。
  * 通过观察矩阵可以知道每个柱子代表的是哪些集合的交集。

1. **左侧柱状图 (Set Size)** ：

* 代表每个集合本身包含的样本数量（set size）。
* **如何查看** ：
  * 高度越高，集合本身的样本越多。
  * 可用于评估集合的重要性（例如，某个集合是否特别大，或是否参与多个交集）。
...
* 查看顶部柱状图中最突出的柱子，找到具有最多样本的交集。
* 对应底部矩阵，确认这些交集涉及哪些集合。

1. **分布趋势** ：

* 通过比较不同柱子的高度，可以发现数据的分布特性。
  * 如果柱子的分布集中在左侧，表示样本集中于少数交集。
  * 如果分布均匀，说明样本在多个交集之间分散。

1. **单集合 vs 多集合交集** ：

* 查看底部矩阵时，注意单点（仅一个黑点）对应的是单个集合本身的大小。
* 连续连接的黑点则表示多个集合之间的交集。

1. **集合覆盖度** ：

* 结合左侧柱状图，查看每个集合的总大小（set size）和它参与的交集数量。
* 如果某个集合参与了多个重要的交集，可能值得进一步分析。
