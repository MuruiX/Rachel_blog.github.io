...
可视化交叉分类涉及显示两个或多个分类变量之间的关系。交叉分类通常以表格或图表的形式表示，有助于揭示数据中的模式或依赖性。


1. Creating a Cross-Classification Table 創建交叉分類表

The first step is to summarize the data into a **contingency table** (cross-tabulation). This shows the counts or frequencies of combinations of categories.
...
2.1 Heatmap
    ...
    Heatmaps are effective for visualizing the counts or percentages in cross-classification tables.
...
- **Annot** : Displays the counts in each cell. 显示每个单元格中的计数
- **Cmap** : Adjusts the color scheme to highlight differences.调整配色方案以突出显示差异。
...
A grouped bar plot shows side-by-side bars for each combination, providing an alternative view.
...
3. Adding Statistical Insights添加统计洞察
